<div class="py-5 container-fluid">
        <div class="container">
            <div class="col-lg-6">
                <div class="mb-5 row align-items-center">
                    <div class="col-4 col-sm-3">
                        <img class="mb-3 w-100 mb-sm-0" src="img/impresora.jpeg" alt="Empleada Miriam Silvestre" width="200" height="150">
                    </div> 
                    <div class="col-8 col-sm-9">
                        <h4>Miriam Silvestre</h4>
                        <p class="m-0">Encargada del Centro de Computaci&oacute;n del Turno Matutino del Bachillerato 8.
                        </p>
                    </div>
                </div>
                
                <div class="mb-5 row align-items-center">
                    <div class="col-4 col-sm-3">
                        <img class="mb-3 w-100 mb-sm-0" src="img/impresora.jpeg" alt="Empleado Carlos Barraza" width="200" height="150">
                    </div>
                    <div class="col-8 col-sm-9">
                        <h4>Carlos Barraza</h4>
                        <p class="m-0">Auxiliar en Servicio del Centro de Computaci&oacute;n del Turno Vespertino del Bachillerato 9.
                        </p>
                    </div>
                </div>
                
            </div>
        </div>
    </div>